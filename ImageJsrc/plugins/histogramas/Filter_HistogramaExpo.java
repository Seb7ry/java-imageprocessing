...
public class Filter_HistogramaExpo implements PlugInFilter {
    ImagePlus imp;
    ...
    public void run(ImageProcessor ip) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                double newPixel = rmin - (1 / alpha) * Math.log(1 - q);
    ...
    private void showInterface() {
        ...
        panel.add(rminField);
        panel.add(new JLabel("α (alfa):"));
        panel.add(alphaField);

        int result = JOptionPane.showConfirmDialog(null, panel, "Parámetros de transformación exponencial", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
