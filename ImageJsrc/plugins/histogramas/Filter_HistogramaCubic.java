...
public class Filter_HistogramaCubic implements PlugInFilter {
    ImagePlus imp;
    ...
    public void run(ImageProcessor ip) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                double r_cubico = Math.pow((Math.cbrt(rmax) - Math.cbrt(rmin)) * q + Math.cbrt(rmin), 3);

                r_cubico = Math.max(0, Math.min(255, r_cubico));

                ip.putPixel(x, y, (int) r_cubico);
            }
        }
    ...
    private void showInterface() {
        ...
        int result = JOptionPane.showConfirmDialog(null, panel, "Parámetros de transformación cúbica", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
